      ...
      <div className="mx-auto flex max-w-7xl gap-6 px-4 py-6 md:px-6 md:py-10">
        ...
        <div className="min-w-0 flex-1">
          <div className="sticky top-16 z-30 mb-6 lg:hidden">
            <div className="overflow-hidden rounded-[30px] border border-[color:var(--border)] bg-[linear-gradient(180deg,_color-mix(in_srgb,var(--surface)_94%,transparent),_color-mix(in_srgb,var(--surface-2)_96%,transparent))] shadow-[0_18px_40px_rgba(25,22,18,0.12)] backdrop-blur">
              <div className="border-b border-[color:var(--border)] px-4 py-4">
                <div className="flex items-start gap-3">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[color:var(--surface)] text-[color:var(--primary)] shadow-sm">
                    {currentItem.icon}
                  </div>
                  <div className="min-w-0">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[color:var(--muted)]">{t('dashboard.mobile_current')}</p>
                    <h2 className="mt-1 truncate text-lg font-semibold text-[color:var(--ink)]">{getNavLabel(t, currentItem.key)}</h2>
                    <p className="mt-1 text-sm leading-6 text-[color:var(--muted)]">{getNavDescription(t, currentItem.key)}</p>
                  </div>
                </div>
              </div>

              <div className="px-3 py-3">
                <p className="px-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-[color:var(--muted)]">{t('dashboard.mobile_jump')}</p>
...
function SidebarNav({
  ...
  return (
    <nav className={mobile ? 'mt-3 flex gap-2 overflow-x-auto pb-1' : 'space-y-2'}>
      {navItems.map((item) => {
        ...
        return (
          <Link
            ...
            className={`border transition-colors ${
              mobile
                ? `group min-w-fit shrink-0 rounded-[20px] px-3 py-3 ${
                    active
                      ? 'border-[color:var(--primary)] bg-[color:var(--primary)] text-white shadow-[0_12px_28px_rgba(108,90,74,0.24)]'
          ...
          >
            {mobile ? (
              <div className="flex items-center gap-2.5">
                ...
                <span className={`max-w-[150px] text-sm font-semibold leading-5 ${
